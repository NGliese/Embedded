...
MQTT_Message::MQTT_Message(const uint64_t& entity_id, const size_t& buffer_size)
	: m_entity_id{entity_id}, m_buffer_size{buffer_size}, m_semaphore{"mqtt message"}
{
#ifdef __ESP32__
	m_semaphore.give();
#endif
...
MQTT_Message::MQTT_Message(const msg_conf_t& conf)
	: m_entity_id{map.at(conf.entity_id)}, m_buffer_size{conf.buffer_size}, m_semaphore{
																				"mqtt message"}
{
#ifdef __ESP32__
	m_semaphore.give();
...
general_err_t MQTT_Message::addData(const uint64_t& time, const uint64_t& data)
{
#ifdef DEBUG
	LOG_PRINT_INFO(LOG_TAG, ">> MQTT_Message::addData >> ");
#endif
...
MQTT_Message MQTT_Message::construct(const std::string& str)
{
	...
	if(vec.empty() or vec.size() < CONSTRUCT_MIN_SIZE or
	   ((vec.size() - CONSTRUCT_PARAMETER_OFFSET) % CONSTRUCT_EXPECTED_MOD))
	{
...
general_err_t MQTT_Message::append(const MQTT_Message& msg)
{
#ifdef DEBUG
