...
class Actuator_Base
{
  ...
  public:
	 ...
	 */
	inline const U& getValue(void) const
	{
		return m_value;
	};
	/**
  ...
  protected:
	/**
	 * Setpoint function, the derived class is able to define its own function to accommodate fault
	 * handling
	 * @attention: no fault handling applied in the default function
	 */
	virtual general_err_t setPoint(const U& value)
	{
		m_value = value;
		return GE_OK;
	};
