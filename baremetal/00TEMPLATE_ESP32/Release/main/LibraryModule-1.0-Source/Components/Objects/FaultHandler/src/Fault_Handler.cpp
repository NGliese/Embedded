...
Fault_Handler::Fault_Handler(const std::string& class_name)
	: m_name{class_name}, m_counters{0, 0, 0}
{
}
...
std::string Fault_Handler::toString(const FaultHandler_n::msg_t& msg)
{
	...
	return severity2string(msg.level) + "," + m_name + "," + msg.function_name + "," +
		   msg.describtion;
}
...
std::string Fault_Handler::severity2string(const FaultHandler_n::severity& level)
{
	switch(level)
	{
		case FaultHandler_n::severity::lowest:
