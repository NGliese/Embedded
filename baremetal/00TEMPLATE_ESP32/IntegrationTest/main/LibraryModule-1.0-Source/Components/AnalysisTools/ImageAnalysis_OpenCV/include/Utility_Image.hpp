...
#endif
public:
    enum class loader_conf{color,gray};
    enum class object_color{GREEN,RED,BLUE};
    static cv::Mat load_image(const std::string& path ,const loader_conf& conf = loader_conf::color);
    ...
    static void saveImage(const timeval& time, const cv::Mat & m,const std::string& name, const std::string & path);
    ...
    static general_err_t putMask(const cv::Mat& src, cv::Mat & dist,  const cv::Point& start, const cv::Point& end);
