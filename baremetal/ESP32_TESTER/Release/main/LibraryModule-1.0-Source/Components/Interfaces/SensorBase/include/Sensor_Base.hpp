...
{
  public:
	Sensor_Base(const T& conf, const std::string& class_name = "N/A")
		: m_value{0}, m_conf{conf}, m_faultHanlder{class_name} {};
	virtual ~Sensor_Base(){};

	virtual general_err_t measure(void) = 0;
	inline const U& getValue(void)
	{
		return m_value;
	};
	/* @brief Handle all parts of the sensor by measuring and returning value in one.
	 * @warning error messages in measure() is neglected here, and should be considered as a fail
	 * safe solution.
	 */
