...
ConnectedComponents::container_t Background_Extractor::statsConnectedComponents(
        const cv::Mat &raw) {
    // update ref for images
    m_previous = m_current;
    m_current  = raw;
...
void Background_Extractor::view(void) const {
    ...
    if(!m_background.empty()) Utility_Image::view_image(Utility_Image::zoom(m_background,m_conf.zoom),name+"background");
    if(!m_forground.empty())  Utility_Image::view_image(Utility_Image::zoom(m_forground,m_conf.zoom),name+"forground");
    if(!m_current.empty())    Utility_Image::view_image(Utility_Image::zoom(m_current,m_conf.zoom),name+"m_current");
    if(!m_previous.empty())   Utility_Image::view_image(Utility_Image::zoom(m_previous,m_conf.zoom),name+"m_previous");
