 ...
 */
WiFi::WiFi() : ip(0), gw(0), netmask(0), m_pWifiEventHandler(nullptr)
{
...
{
	ESP_LOGD(LOG_TAG, "Setting DNS[%d] to %d.%d.%d.%d", m_dnsCount, ((uint8_t*)(&ip))[0],
 ...
 *
 * Here we define a server that will act as a DNS server.  We use numdns to specify which DNS server
 * to set
 *
 * For example:
 *
 ...
 * @param [in] mode WIFI_MODE_AP for normal or WIFI_MODE_APSTA if you want to keep an Access Point
 * running while you connect
 * @return ESP_OK if we are now connected and wifi_err_reason_t if not.
 */
uint8_t WiFi::connectAP(const std::string& ssid, const std::string& password,
						bool waitForConnection, wifi_mode_t mode)
{
	LOG_PRINT_INFO(LOG_TAG, ">> WiFi::connectAP >> ");
	...
	if(errRc != ESP_OK)
	{
		// ESP_LOGE(LOG_TAG, "esp_wifi_set_mode: rc=%d %s", errRc,
	...
	if(errRc != ESP_OK)
	{
		//	ESP_LOGE(LOG_TAG, "esp_wifi_start: rc=%d %s", errRc,
		// GeneralUtils::errorToString(errRc));
		abort();
	}
	...
	do
	{
		...
		if(errRc != ESP_OK)
		{
			//	ESP_LOGE(LOG_TAG, "esp_wifi_connect: rc=%d %s", errRc,
			//			 GeneralUtils::errorToString(errRc));
			abort();
		}
	...
	LOG_PRINT_INFO(LOG_TAG, "<< WiFi::connectAP <<");
	return m_apConnectionStatus; // Return ESP_OK if we are now connected and wifi_err_reason_t if
								 // not.
} // connectAP
...
{
	return (m_apConnectionStatus == ESP_OK);
} // isConnected
...
{
	// This is the common event handler that we have provided for all event processing.  It is
	// called for every event that is received by the WiFi subsystem.  The "ctx" parameter is an
	// instance of the current WiFi object that we are processing.  We can then retrieve the
	// specific/custom event handler from within it and invoke that.  This then makes this an
	// indirection vector to the real caller.
	...
	// If the event we received indicates that we now have an IP address or that a connection was
	// disconnected then unlock the mutex that indicates we are waiting for a connection complete.
	if(event->event_id == SYSTEM_EVENT_STA_GOT_IP ||
	   event->event_id == SYSTEM_EVENT_STA_DISCONNECTED)
	{
		if(event->event_id == SYSTEM_EVENT_STA_GOT_IP)
		{ // If we connected and have an IP, change the status to ESP_OK.  Otherwise, change it to
		  // the reason code.
...
std::string WiFi::getApMac()
{
	...
	sprintf(mac_str, "%02X:%02X:%02X:%02X:%02X:%02X", mac[0], mac[1], mac[2], mac[3], mac[4],
			mac[5]);
...
/* PRIVATE */ void WiFi::init()
{
	...
	{
		esp_err_t errRc =
			::esp_event_loop_init(WiFi::eventHandler, this); // Initialze the event handler.
		if(errRc != ESP_OK)
		{
			//	ESP_LOGE(LOG_TAG, "esp_event_loop_init: rc=%d %s", errRc,
			//			 GeneralUtils::errorToString(errRc));
			abort();
		}
	...
	if(!m_initCalled)
	{
		...
		if(errRc != ESP_OK)
		{
			//	ESP_LOGE(LOG_TAG, "esp_wifi_init: rc=%d %s", errRc,
			// GeneralUtils::errorToString(errRc));
			abort();
		}
		...
		if(errRc != ESP_OK)
		{
			//	ESP_LOGE(LOG_TAG, "esp_wifi_set_storage: rc=%d %s", errRc,
			//				 GeneralUtils::errorToString(errRc));
			abort();
		}
	}
...
std::vector<WiFiAPRecord> WiFi::scan()
{
	...
	if(errRc != ESP_OK)
	{
		//	ESP_LOGE(LOG_TAG, "esp_wifi_scan_get_ap_records: rc=%d %s", errRc,
		//			 GeneralUtils::errorToString(errRc));
		abort();
	}
				   ...
				   uint8_t channel, bool ssid_hidden, uint8_t max_connection)
{
	...
	if(errRc != ESP_OK)
	{
		//	ESP_LOGE(LOG_TAG, "tcpip_adapter_dhcps_start: rc=%d %s", errRc,
		//			 GeneralUtils::errorToString(errRc));
	}
...
void MDNS::serviceInstanceSet(const std::string& service, const std::string& proto, const
std::string& instance) { serviceInstanceSet(service.c_str(), proto.c_str(), instance.c_str()); } //
serviceInstanceSet
...
void MDNS::serviceInstanceSet(const char* service, const char* proto, const char* instance) {
  ...
  if (errRc != ESP_OK) {
	ESP_LOGE(LOG_TAG, "mdns_service_instance_set: rc=%d %s", errRc,
GeneralUtils::errorToString(errRc)); abort();
  }
} // serviceInstanceSet
...
void MDNS::serviceRemove(const char* service, const char* proto) {
	...
	if (errRc != ESP_OK) {
		ESP_LOGE(LOG_TAG, "mdns_service_remove: rc=%d %s", errRc,
GeneralUtils::errorToString(errRc)); abort();
	}
} // serviceRemove
*/
