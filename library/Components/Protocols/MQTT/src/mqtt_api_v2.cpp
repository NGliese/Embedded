...
// OTA m_ota;
mqttclient m_client;
static std::map<std::string, general_err_t (*)(const char* data, const size_t data_length)>
	mqtt_subscriber_pointer_map;
...
general_err_t mqtt_api_v2::connect(void)
{
	...
	auto err = esp_mqtt_client_register_event(m_client, (esp_mqtt_event_id_t)ESP_EVENT_ANY_ID,
											  &(mqtt_api_v2::mqtt_event_handler), m_client);
...
general_err_t mqtt_api_v2::publish(const std::string& topic, const std::string& payload)
{
#ifdef DEBUG
...
general_err_t mqtt_api_v2::subscribe(const std::string& topic,
																	   ...
																	   const size_t data_length))
{
	...
	// Executable code:
#ifdef __ESP32__
	auto err = esp_mqtt_client_subscribe(m_client, topic.c_str(), 0);
	if(err != ESP_OK)
	{
		return GE_FAIL;
	}
#endif
	mqtt_subscriber_pointer_map.insert(
		std::pair<const std::string, general_err_t (*)(const char* data, const size_t data_length)>(
			topic, callbackfunction));
...
general_err_t mqtt_api_v2::handle_event(const char* topic, const size_t topic_length,
										const char* data, const size_t data_length)
{
#ifdef DEBUG
	LOG_PRINT_INFO(LOG_TAG, ">> mqtt_api_v2::handle_event >> ");
#endif
	// Executable code:
	std::string topic_string{topic, topic_length};
	std::cout << " recieved topic is : " << topic_string << "\n";

	// lets make sure that we have the topic in our map
	if(mqtt_subscriber_pointer_map.end() == mqtt_subscriber_pointer_map.find(topic_string))
	{
		// we could not find the topic
		std::cout << " topic not found : " << topic_string << "\n";
		return GE_FAIL;
	}
	// if we do, lets call the callback function
	auto err = mqtt_subscriber_pointer_map.at(topic_string)(data, data_length);
	if(err != GE_OK)
	{
		return err;
	}
...
esp_err_t mqtt_api_v2::event_handler(mqttevent event)
{
	...
	switch(event->event_id)
	{
		...
		case MQTT_EVENT_DATA:
			...
			OTA::update(event->data, event->data_len);
			// mqtt_api_v2::handle_event(event->topic,event->topic_len,event->data,event->data_len);
...
#endif
#ifdef __ESP32__
void mqtt_api_v2::mqtt_event_handler(void* handler_args, esp_event_base_t base, int32_t event_id,
									 void* event_data)
{
