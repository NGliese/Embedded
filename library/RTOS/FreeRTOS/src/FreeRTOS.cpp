...
FreeRTOS::Semaphore::Semaphore(std::string name) :m_usePthreads{false} {
#ifdef __ESP32__
	if (m_usePthreads) {
	...
	//	m_semaphore = xSemaphoreCreateMutex();
		/*
		 * apperently a bug has been happening in the FreeRTOS due to Mutex, so maybe binary semaphore is
		 * acceptable, -> usually semaphore is used as a single lock, so why not?
...
bool FreeRTOS::Semaphore::take(uint32_t timeoutMs, std::string owner) {
	...
	bool rc = false;
	if (m_usePthreads) {
		ESP_LOGW(LOG_TAG, "We apparently don't have a timed wait for pthreads.  %s", toString().c_str());
	//	assert(false);  // We apparently don't have a timed wait for pthreads. // @suppress("Function cannot be resolved")
	} else {
