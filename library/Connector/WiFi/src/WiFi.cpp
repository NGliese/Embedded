...
uint8_t WiFi::connectAP(const std::string& ssid, const std::string& password, bool waitForConnection, wifi_mode_t mode){
 //   LOG_PRINT_INFO(LOG_TAG, ">> WiFi::connectAP >> ");
	...
	if (errRc != ESP_OK) {
		ESP_LOGE(LOG_TAG, "esp_wifi_set_mode: rc=%d ", errRc);
		abort();
	}
	...
	if (errRc != ESP_OK) {
		ESP_LOGE(LOG_TAG, "esp_wifi_set_config: rc=%d ", errRc);
		abort();
	}
	errRc = ::esp_wifi_start();
	if (errRc != ESP_OK) {
		ESP_LOGE(LOG_TAG, "esp_wifi_start: rc=%d ", errRc);
		abort();
	}
...
//	    LOG_PRINT_INFO(LOG_TAG, "esp_wifi_connect");
		...
		if (errRc != ESP_OK) {
			ESP_LOGE(LOG_TAG, "esp_wifi_connect: rc=%d ", errRc );
			abort();
		}
	}
...
/* PRIVATE */ void WiFi::init() {
	...
	if (m_eventLoopStarted) {
		...
		if (errRc != ESP_OK) {
			ESP_LOGE(LOG_TAG, "esp_event_loop_init: rc=%d ", errRc);
			abort();
		}
	...
	if (!m_initCalled) {
		...
		if (errRc != ESP_OK) {
			ESP_LOGE(LOG_TAG, "esp_wifi_init: rc=%d ", errRc);
			abort();
		}

		errRc = ::esp_wifi_set_storage(WIFI_STORAGE_RAM);
		if (errRc != ESP_OK) {
			ESP_LOGE(LOG_TAG, "esp_wifi_set_storage: rc=%d ", errRc);
			abort();
		}
	}
	m_initCalled = true;
...
std::vector<WiFiAPRecord> WiFi::scan() {
	...
	if (errRc != ESP_OK) {
		ESP_LOGE(LOG_TAG, "esp_wifi_scan_get_ap_records: rc=%d ", errRc);
		abort();
	}
...
void WiFi::startAP(const std::string& ssid, const std::string& password, wifi_auth_mode_t auth, uint8_t channel, bool ssid_hidden, uint8_t max_connection) {
	...
	if (errRc != ESP_OK) {
		ESP_LOGE(LOG_TAG, "tcpip_adapter_dhcps_start: rc=%d ", errRc);
	}
