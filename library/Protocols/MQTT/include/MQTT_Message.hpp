...
public:
    ...
    MQTT_Message(const uint64_t& entity_id, const size_t & buffer_size = 1) ;
    MQTT_Message(const msg_conf_t& conf) ;
    ~MQTT_Message();
    general_err_t addData(const uint64_t& data);
    general_err_t addData_f(const float& data);
    inline bool isEmpty(void) { return m_data.empty();};
    const std::string toString(void); // copy
    general_err_t clear(void);
    auto getBufferSize(void) { return m_data.size();}
...
public:
    ...
    friend_MQTT_Message(MQTT_Message * MQTT_Message) : m_sensor{MQTT_Message} { };

    ~friend_MQTT_Message(){};
    std::vector<std::pair<uint64_t,uint64_t>>  * getbuffer() {return &(m_sensor->m_data); }
    auto getID(void) {return m_sensor->m_entity_id;}
private:
